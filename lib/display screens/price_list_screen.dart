...
class _PriceListScreenState extends State<PriceListScreen> {
  ...
  Future<void> _uploadDocument() async {
    try {
      ...
      if (user == null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Please log in to upload price lists', style: GoogleFonts.poppins())),
          );
        }
        return;
      }
      ...
      logger.i('Attempting to write to PriceList collection for user: ${user.uid}');
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Price list uploaded successfully', style: GoogleFonts.poppins())),
        );
        _titleController.clear();
      }
      logger.i('Uploaded price list: $fileName, URL: $downloadUrl, facilityId: ${widget.facilityId}');
    } catch (e, stackTrace) {
      logger.e('Error uploading price list: $e', stackTrace: stackTrace);
      if (mounted) {
        Navigator.pop(context); // Close progress dialog
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error uploading price list: $e', style: GoogleFonts.poppins())),
        );
      }
    } finally {
      setState(() {
  ...
  Future<void> _viewDocument(String url, String fileName) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error viewing price list: $e', style: GoogleFonts.poppins())),
        );
      }
    }
  }
  ...
  Future<void> _downloadDocument(String url, String fileName) async {
    try {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        if (await file.exists()) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Price list downloaded to $filePath', style: GoogleFonts.poppins()),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error downloading price list: $e', style: GoogleFonts.poppins())),
        );
      }
    }
  }
  ...
  Future<void> _deleteDocument(String downloadUrl, String fileName, String docId) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Price list deleted successfully', style: GoogleFonts.poppins())),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error deleting price list: $e', style: GoogleFonts.poppins())),
        );
      }
    }
  }
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (docs.isEmpty) {
                      ...
                      return Padding(
                        ...
                        child: Text('No price lists uploaded yet', style: GoogleFonts.poppins()),
                      );
                    }
